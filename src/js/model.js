...
export const loadRecipe = async function (id) {
  try {
    ...
    // If there is any bookmark, which has the bookmark ID equal to the ID that we just received, then we want the current recipe which is state.recipe to be bookmarked
...
export const loadSearchResults = async function (query) {
  try {
    ...
    state.search.page = 1; // For reset pages in new search
  } catch (err) {
    console.error(`${err} 💥💥💥💥`);
    throw err;
  }
...
export const deleteBookmark = function (id) {
  // Delete the recipe from the bookmark array
...
// Takeing bookmarks out
const init = function () {
  const storage = localStorage.getItem('bookmarks'); // clean up the localStorage
  if (storage) state.bookmarks = JSON.parse(storage); // converted string back to an object
};
init();
